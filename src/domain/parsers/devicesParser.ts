import { SysDevice, SysResource, SysBlock } from "../sysModel";
...
export function parseDevices(
  ...
  for (const device of deviceArray) {
    ...
    if (device.Parameter) {
      const paramList = Array.isArray(device.Parameter) ? device.Parameter : [device.Parameter];
    ...
    if (device.Attribute) {
      const attrList = Array.isArray(device.Attribute) ? device.Attribute : [device.Attribute];
    ...
    if (device.Resource) {
      const resourceList = Array.isArray(device.Resource) ? device.Resource : [device.Resource];
      ...
      for (const res of resourceList) {
        ...
        if (res.FBNetwork?.FB) {
          const fbList = Array.isArray(res.FBNetwork.FB) ? res.FBNetwork.FB : [res.FBNetwork.FB];
