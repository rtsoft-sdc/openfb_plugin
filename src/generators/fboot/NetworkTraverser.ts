...
export class NetworkTraverser {
  ...
  private traverseNetwork(
    ...
    if (network.blocks) {
      for (const block of network.blocks) {
        const qualifiedName = this.qualifyName(namespace, block.id);
        context.fbs.push({
          qualifiedName,
    ...
    if (network.subApps) {
      for (const subApp of network.subApps) {
        const qualifiedName = this.qualifyName(namespace, subApp.id);
      ...
      for (const conn of network.connections) {
        const fromQualified =
          conn.fromBlock === SUBAPP_INTERFACE_BLOCK
            ? namespace
            : this.qualifyName(namespace, conn.fromBlock);
        const toQualified =
          conn.toBlock === SUBAPP_INTERFACE_BLOCK
            ? namespace
            : this.qualifyName(namespace, conn.toBlock);
  ...
  /**
   * Qualifies a local name with the namespace.
   * Returns the local name if namespace is empty or if name is already qualified.
   */
